...
;
    ...
    task test_instructions_validity();
        for (int i = 0; i < 2; ++i) begin
            ...
            #1;
            assert (instr_valid == ibus_rvalid)
            else $error("instr_valid: exp: %b, rcv: %b", ibus_rvalid, instr_valid);
        end
    endtask
    ...
    task test_r_type_instructions();
        ...
        foreach (instructions[i]) begin
            ...
            else $error("instr: %s: instr: exp: %s, rcv: %s", instructions[i].name, instructions[i].name, instr.name);

            assert ((rs1 == ibus_rdata[19:15]) && (rs2 == ibus_rdata[24:20]) && (rd == ibus_rdata[11:7]))
            else
                $error(
                    "instr: %s: rs1: exp: %x, rcv: %x; rs2: exp: %x, rcv: %x; rd: exp: %x, rcv: %x",
                    instructions[i].name,
                    ibus_rdata[19:15],
                    rs1,
                    ibus_rdata[24:20],
    ...
    task test_i_type_instructions();
        static instr_t instructions[$] = {JALR, LB, LH, LW, LBU, LHU, ADDI, SLTI, SLTIU, XORI, ORI, ANDI};
        ...
        foreach (instructions[i]) begin
            ...
            assert (imm == {{20{ibus_rdata[31]}}, ibus_rdata[31:20]})
            else
                $error(
                    "instr: %s: imm: exp: %x, rcv: %x",
                    instructions[i].name,
                    {
                        {20{ibus_rdata[31]}}, ibus_rdata[31:20]
                    },
                    imm
                );

            assert ((rs1 == ibus_rdata[19:15]) && (rd == ibus_rdata[11:7]))
            else
                $error(
                    "instr: %s: rs1: exp: %x, rcv: %x; rd: exp: %x, rcv: %x",
                    instructions[i].name,
                    ibus_rdata[19:15],
                    rs1,
                    ibus_rdata[11:7],
    ...
    task test_s_type_instructions();
        ...
        foreach (instructions[i]) begin
            ...
            assert (imm == {{20{ibus_rdata[31]}}, ibus_rdata[31:25], ibus_rdata[11:7]})
            else
                $error(
                    "instr: %s: imm: exp: %x, rcv: %x",
                    instructions[i].name,
                    {
                        {20{ibus_rdata[31]}}, ibus_rdata[31:25], ibus_rdata[11:7]
                    },
                    imm
                );

            assert ((rs1 == ibus_rdata[19:15]) && (rs2 == ibus_rdata[24:20]))
            else
                $error(
                    "instr: %s: rs1: exp: %x, rcv: %x; rs2: exp: %x, rcv: %x",
                    instructions[i].name,
                    ibus_rdata[19:15],
                    rs1,
                    ibus_rdata[24:20],
    ...
    task test_b_type_instructions();
        ...
        foreach (instructions[i]) begin
            ...
            assert (imm == {{19{ibus_rdata[31]}}, ibus_rdata[31], ibus_rdata[7], ibus_rdata[30:25], ibus_rdata[11:8], 1'b0})
            else
                $error(
                    "instr: %s: imm: exp: %x, rcv: %x",
                    instructions[i].name,
                    {
                        {19{ibus_rdata[31]}}, ibus_rdata[31], ibus_rdata[7], ibus_rdata[30:25], ibus_rdata[11:8], 1'b0
                    },
                    imm
                );
    ...
    task test_u_type_instructions();
        ...
        foreach (instructions[i]) begin
            ...
            assert (imm == {ibus_rdata[31:12], 12'b0})
            else $error("instr: %s: imm: exp: %x, rcv: %x", instructions[i].name, {ibus_rdata[31:12], 12'b0}, imm);

            assert (rd == ibus_rdata[11:7])
            else $error("instr: %s: rd: exp: %x, rcv: %x", instructions[i].name, ibus_rdata[11:7], rd);
        end
    endtask
    ...
    task test_j_type_instructions();
        ...
        foreach (instructions[i]) begin
            ...
            assert (imm == {{12{ibus_rdata[31]}}, ibus_rdata[19:12], ibus_rdata[20], ibus_rdata[30:21], 1'b0})
            else
                $error(
                    "instr: %s: imm: exp: %x, rcv: %x",
                    instructions[i].name,
                    {
                        {12{ibus_rdata[31]}}, ibus_rdata[19:12], ibus_rdata[20], ibus_rdata[30:21], 1'b0
                    },
                    imm
                );
