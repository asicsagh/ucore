...
package instr_pkg;
    ...
    typedef struct {logic [6:0] opcode;} u_type_instr_code_t;

    typedef struct {logic [6:0] opcode;} j_type_instr_code_t;
    ...
    function logic [31:0] get_r_type_instruction(r_type_instr_code_t code, logic [4:0] rd, logic [4:0] rs1,
                                                 logic [4:0] rs2);
        return {code.funct7, rs2, rs1, code.funct3, rd, code.opcode};
    endfunction

    function logic [31:0] get_i_type_instruction(i_type_instr_code_t code, logic [4:0] rd, logic [4:0] rs1,
                                                 logic [11:0] imm);
        return {imm, rs1, code.funct3, rd, code.opcode};
    endfunction

    function logic [31:0] get_s_type_instruction(s_type_instr_code_t code, logic [4:0] rs1, logic [4:0] rs2,
                                                 logic [11:0] imm);
        return {imm[11:5], rs2, rs1, code.funct3, imm[4:0], code.opcode};
    endfunction

    function logic [31:0] get_b_type_instruction(b_type_instr_code_t code, logic [4:0] rs1, logic [4:0] rs2,
                                                 logic [12:0] imm);
        return {imm[12], imm[10:5], rs2, rs1, code.funct3, imm[4:1], imm[11], code.opcode};
    endfunction

    function logic [31:0] get_u_type_instruction(u_type_instr_code_t code, logic [4:0] rd, logic [31:0] imm);
        return {imm[31:12], rd, code.opcode};
    endfunction

    function logic [31:0] get_j_type_instruction(j_type_instr_code_t code, logic [4:0] rd, logic [20:0] imm);
