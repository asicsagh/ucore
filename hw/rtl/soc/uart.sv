...
module uart (
    ...
    const
    logic [11:0]
        CR_OFFSET = 12'h000, SR_OFFSET = 12'h004, CCR_OFFSET = 12'h008, WDR_OFFSET = 12'h00c, RDR_OFFSET = 12'h010;
    ...
    always_comb begin
        ...
        if (dbus.rreq && (dbus.addr[11:0] == RDR_OFFSET)) sr_nxt[0] = 1'b0;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) dbus.rdata <= 32'b0;
        else dbus.rdata <= dbus_rdata_nxt;
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) tx_data_valid <= 1'b0;
        else tx_data_valid <= dbus.wreq && (dbus.addr[11:0] == WDR_OFFSET);
    end
