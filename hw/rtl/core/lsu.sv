...
(
        ...
        case (state)
            IDLE: begin
                if (op != LSU_NONE_OP) begin
                    if (!dbus.stall) state_nxt = ACTIVE;
                end
            end
            ACTIVE: begin
                if (!dbus.stall) state_nxt = IDLE;
            end
        endcase
    end
    ...
    always_comb begin
        dbus.rreq = op inside {LSU_LOAD_BYTE, LSU_LOAD_BYTE_UNSIGNED,
        LSU_LOAD_HALF_WORD, LSU_LOAD_HALF_WORD_UNSIGNED, LSU_LOAD_WORD};
    end

    always_comb begin
        dbus.wreq = op inside {LSU_STORE_BYTE, LSU_STORE_HALF_WORD, LSU_STORE_WORD};
    end
