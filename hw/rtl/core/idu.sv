...
(
    ...
    function logic is_i_type(instr_t instr);
        return instr inside {JALR, LB, LH, LW, LBU, LHU, ADDI, SLTI, SLTIU, XORI, ORI, ANDI, SLLI, SRLI, SRAI};
    endfunction
    ...
    always_comb begin
        if (is_r_type(instr)) imm = 32'b0;
        else if (is_i_type(instr)) imm = {{20{ibus_rdata[31]}}, ibus_rdata[31:20]};
        else if (is_s_type(instr)) imm = {{20{ibus_rdata[31]}}, ibus_rdata[31:25], ibus_rdata[11:7]};
        else if (is_b_type(instr))
            imm = {{19{ibus_rdata[31]}}, ibus_rdata[31], ibus_rdata[7], ibus_rdata[30:25], ibus_rdata[11:8], 1'b0};
        else if (is_u_type(instr)) imm = {ibus_rdata[31:12], 12'b0};
